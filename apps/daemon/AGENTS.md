...
- **Do not import from `apps/website/*`** — the daemon must stay stateless and decoupled from the UI.
...
- **Build target**: `bunx esbuild src/index.ts --bundle --platform=node --outfile=dist/daemon.cjs`. Workspace `@mon/*` deps are inlined; runtime needs only `execa` and `node-schedule` if you ever switch to non-bundled mode (currently bundled).
...
- **One file has a non-aliased import**: [`jobs/host.ts`](file:///Users/guneet/projects/mon/apps/daemon/src/jobs/host.ts#L5-L6) uses `"packages/config/schema"` instead of `@mon/config/schema`. Inconsistent with siblings — prefer `@mon/*` when editing.
